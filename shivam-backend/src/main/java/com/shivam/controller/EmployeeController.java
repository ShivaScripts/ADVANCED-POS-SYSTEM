...
public class EmployeeController {
    ...
    @PatchMapping("/{employeeId}/toggle-access")
    @PreAuthorize("hasAnyRole('ROLE_BRANCH_ADMIN', 'ROLE_BRANCH_MANAGER')")
    public ResponseEntity<User> toggleEmployeeAccess(
            @PathVariable Long employeeId
    ) throws Exception {
        User updatedEmployee = employeeService.toggleEmployeeAccess(employeeId);
        return new ResponseEntity<>(updatedEmployee, HttpStatus.OK);
    }
}