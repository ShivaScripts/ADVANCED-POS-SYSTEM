// pos-frontend-vite/src/pages/store/Dashboard/Transactions.jsx

import React, { useState, useEffect } from "react"; // <-- useState and useEffect combined
...
// removed separate useEffect import because it's now in the combined import above
import { getOrdersByBranch, exportTransactionsByBranch } from "../../../Redux Toolkit/features/order/orderThunks"; // <-- Updated import
...
export default function Transactions() {
  ...
  // --- START: ADDED STATE FOR EXPORTING ---
  ...
  // --- START: ADD THIS HANDLER (exports the file via thunk) ---
  const handleExport = async () => {
      ...
      setIsExporting(true);
      toast.info("Exporting transactions, please wait...");
      
      try {
        const result = await dispatch(exportTransactionsByBranch(branch.id)).unwrap();
        toast.success("Transactions exported successfully!", { description: result.filename });
      } catch (error) {
        console.error("Export failed:", error);
        toast.error("Export failed", { description: error || "Could not generate file." });
      } finally {
        setIsExporting(false);
      }
  };
  // --- END: ADD THIS HANDLER ---
  ...
  return (
    <div className="space-y-6">
      <div className="flex justify-between items-center">
        ...
        {/* --- START: UPDATED BUTTON TO USE NEW HANDLER --- */}
        <Button 
