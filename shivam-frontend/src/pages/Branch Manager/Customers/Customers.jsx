...
const Customers = () => {
  ...
  // --- UPDATED SELECTORS ---
  const { customers, overviewStats } = useSelector((state) => state.customer);
  ...
  useEffect(() => {
    ...
    // dispatch(getAllCustomers()); // <-- Removed

    if (branch?.id) {
      dispatch(getCustomersByBranch(branch.id)); // <-- Add this line
      dispatch(getCustomerStatsByBranch(branch.id));
    }
    // --- END: THIS IS THE FIX ---
  }, [dispatch, branch]);
        ...
        <Card>
          <CardContent className="p-6">
            <div className="flex flex-col items-center justify-center">
              ...
              {/* Uses overviewStats from backend */}
              <p className="text-3xl font-bold mt-2">
                {overviewStats?.totalCustomers || 0}
              </p>
            </div>
          </CardContent>
        </Card>
        ...
        <Card>
          <CardContent className="p-6">
            <div className="flex flex-col items-center justify-center">
              ...
              {/* Safe: avoids NaN / crash */}
              <p className="text-3xl font-bold mt-2 text-blue-600">
                {overviewStats?.avgOrdersPerCustomer?.toFixed(1) || "0.0"}
              </p>
            </div>
          </CardContent>
        </Card>
