...
// ... (at the end of getStoreStatusDistribution) ...
...
export const getRecentActivity = createAsyncThunk(
  ...
  async (_, { rejectWithValue }) => {
    try {
      ...
      const headers = getAuthHeaders();
      const res = await api.get('/api/super-admin/dashboard/recent-activity', { headers });
      
      console.log('✅ Recent activity fetched successfully:', res.data);
      
      return res.data; // This will be the list of ActivityLogDTOs
    } catch (err) {
