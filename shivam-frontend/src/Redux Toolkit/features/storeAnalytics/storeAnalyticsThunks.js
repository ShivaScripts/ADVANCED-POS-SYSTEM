// === Merged: storeAnalyticsThunks.js ===
// This file merges your original thunks with the new `getRecentSales` thunk provided by the AI.
// All existing thunks are preserved and the new thunk is appended.
...
// --- ADDITION: Get Recent Sales (top 5) ---
export const getRecentSales = createAsyncThunk(
  "storeAnalytics/getRecentSales",
  async (storeAdminId, { rejectWithValue }) => {
    try {
      console.log('🔄 Fetching recent sales...', { storeAdminId });
      
      const headers = getAuthHeaders();
      const res = await api.get(`/api/store/analytics/${storeAdminId}/recent-sales`, { headers });
